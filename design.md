...
The supervisor cannot directly deny a request outright; it can only acknowledge the notification and allow the syscall to restart.  This avoids the complexity of setting a syscall return value in an architecture-agnostic way.  In the future, direct deny support may be added as an extension.  One possible approach would be to store a "pending denial" flag in the task's Landlock credential security blob (or a per-task variable) and have the LSM hooks check this flag at the start of the restarted syscall.  When the supervisor responds with a deny decision, the kernel would set this flag and associate it with the specific access/path.  On the next syscall restart, the hook would find the pending denial, clear the flag, and return `-EACCES` immediately without re-checking rules.  This would require careful handling to ensure the pending denial is cleared if the task receives a signal or the syscall changes between restarts.
