...
suppress audit logs for specific denied accesses on a per-object basis
with the ``LANDLOCK_ADD_RULE_QUIET`` flag of sys_landlock_add_rule(), in
...
audit logs for scope accesses via the ``quiet_scoped`` field of struct
...
object without this flag does not clear it.
