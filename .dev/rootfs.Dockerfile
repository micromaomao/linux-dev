...
RUN dpkg --add-architecture i386 && apt update && apt install -y \
    bash binutils bpftrace curl dhcpcd fio fish g++ gcc gdb git htop iproute2 kitty-terminfo linux-perf ltrace make net-tools ssh strace sysbench sysstat tcpdump tmux trace-cmd vim wget \
