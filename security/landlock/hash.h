...
/*
 * Landlock - Domain hashtable manipulation
 *
				     ...
				     struct landlock_hashtable *out_ht)
{
	...
	if (expected_num_entries > LANDLOCK_HASH_LINEAR_THRESHOLD) {
		table_size = roundup_pow_of_two(expected_num_entries);
		hash_bits = fls_long(table_size - 1);
	}
	...
	out_ht->hlist = kcalloc(table_size, sizeof(struct hlist_head),
				GFP_KERNEL_ACCOUNT);
	if (!out_ht->hlist) {
		return -ENOMEM;
	}
