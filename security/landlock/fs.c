...
static bool check_supervisor_optional_access_recheck(
	...
	deny_masks_t *out_deny_masks, access_mask_t *out_quiet_access)
{
	...
	if (!allowed && (out_deny_masks || out_quiet_access)) {
		...
		quiet_access = landlock_get_quiet_optional_accesses(
			_LANDLOCK_ACCESS_FS_OPTIONAL, deny_masks, rule_flags);
...
static int hook_file_truncate(struct file *const file)
{
	...
	if (check_supervisor_optional_access_recheck(
		    file, LANDLOCK_ACCESS_FS_TRUNCATE, &deny_masks,
		    &quiet_access))
		return 0;
				  ...
				  const unsigned int cmd, const bool is_compat)
{
	 ...
	 */
	if (check_supervisor_optional_access_recheck(
		    file, LANDLOCK_ACCESS_FS_IOCTL_DEV, &deny_masks,
		    &quiet_access))
		return 0;
